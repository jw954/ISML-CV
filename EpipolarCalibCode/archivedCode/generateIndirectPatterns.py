...
newcameramtx, roi = cv2.getOptimalNewCameraMatrix(mtx, dist, (cam_W,cam_H), 0, (cam_W,cam_H))
...
coord_y, coord_x = np.mgrid[1:(proj_H+1),1:(proj_W+1)].astype("uint16")
...
if flip:
    ...
    primal_ind = primal_coord[:,1].reshape(720,1280);
    ...
    primal_ind = primal_coord[:,0].reshape(720,1280);
...
randshift = np.random.permutation(3);
...
for k in np.arange(1,97):
    ...
    im = Image.fromarray(255*primal_val)
    im = im.convert("1")
    im.save('%s/%02d_PAT.bmp' % (projFolder, k-1))
    
    paddedIm[0:cam_H, 0: cam_W] = np.rot90(dual_val, 2);
    
    im = Image.fromarray(255*paddedIm)
    im = im.convert("1")
    im.save('%s/dual_%03d.bmp' % (maskFolder, k-1))
...
for k in np.arange(1,16):
    for c in range(3):
        ...
        paddedIm[0:cam_H, 0: cam_W] = np.rot90(dual_val[(dual_ind-1).astype("int")], 2);
        
        im = Image.fromarray(255*paddedIm)
        im = im.convert("1")
        im.save('%s/dual_%03d.bmp' % (maskFolder,iter))
